/**
 * Modal d'import générique et réutilisable
 */
...
export default function ImportModal({ 
  ...
  const {
    ...
    modes = [
      { value: 'create', label: 'Créer uniquement', description: 'Créer de nouvelles entrées, ignorer les doublons' },
      { value: 'update', label: 'Mettre à jour uniquement', description: 'Mettre à jour les entrées existantes' },
      { value: 'merge', label: 'Fusionner', description: 'Créer les nouvelles et mettre à jour les existantes' }
    ],
  ...
  const [importMode, setImportMode] = useState(modes[0]?.value || 'create');
  ...
  const handleFile = (file) => {
    ...
    const validExtension = acceptedFormats.some(ext => file.name.endsWith(ext));
    
    if (!validTypes.includes(file.type) && !validExtension) {
      setError(`Format de fichier non supporté. Utilisez: ${acceptedFormats.join(', ')}`);
      return;
    }
    ...
    reader.onload = (e) => {
      try {
        if (file.type === 'application/json' || file.name.endsWith('.json')) {
          ...
          setPreview({
            type: 'json',
            count: Array.isArray(data) ? data.length : (data[entityNamePlural] ? data[entityNamePlural].length : 0),
            sample: Array.isArray(data) ? data[0] : (data[entityNamePlural] ? data[entityNamePlural][0] : null)
          });
        } else {
  ...
  // Fonctions de téléchargement de templates
  const downloadTemplateJSON = () => {
    if (templateGenerator) {
      const content = templateGenerator('json');
      const blob = new Blob([JSON.stringify(content, null, 2)], { type: 'application/json' });
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `template-import-${entityNamePlural}.json`;
  ...
  const downloadTemplateCSV = () => {
    if (templateGenerator) {
      const content = templateGenerator('csv');
      const blob = new Blob([content], { type: 'text/csv;charset=utf-8;' });
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `template-import-${entityNamePlural}.csv`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);
        ...
        <div className="p-6 space-y-6">
            ...
            {!file ? (
              <div className="text-center">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-4">
                  Formats supportés: {acceptedFormats.join(', ').toUpperCase()}
                </p>
              </div>
            ) : (
              <div className="text-center">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-green-700 dark:text-green-400">
                    {preview.count} {entityName}{preview.count > 1 ? 's' : ''} trouvé{preview.count > 1 ? 's' : ''}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-3">
                {modes.map(mode => (
                  <label
                    key={mode.value}
                    className={`flex items-start gap-3 p-4 rounded-lg border-2 cursor-pointer transition-all hover:bg-gray-50 dark:hover:bg-gray-700/50 ${
          ...
          <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
            <p className="text-sm text-blue-800 dark:text-blue-300">
              ℹ️ Format attendu: <code className="bg-blue-100 dark:bg-blue-900 px-2 py-1 rounded">{exampleFormat}</code>
            </p>
          </div>
        </div>
